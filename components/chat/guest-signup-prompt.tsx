...
const benefits = [
  "Unlimited software engineering Q&A after sign-in",
  "Chat history saved across devices",
  "Code debugging, reviews, and architecture help",
  "Access on any device with your account",
];
    ...
    <AlertDialog onOpenChange={onOpenChange} open={open}>
      <AlertDialogContent className="max-w-md">
        <AlertDialogHeader>
          ...
          <AlertDialogTitle>You&apos;ve used your 5 free messages</AlertDialogTitle>
          <AlertDialogDescription>
            Sign in or create a free account to keep chatting with Codev about
            software engineering, coding, and debugging.
          </AlertDialogDescription>
        </AlertDialogHeader>
        ...
        <ul className="my-2 space-y-2 text-sm text-foreground/90">
          {benefits.map((benefit) => (
            <li className="flex items-start gap-2" key={benefit}>
              <LockIcon className="mt-0.5 size-3.5 shrink-0 text-red-400" />
              <span>{benefit}</span>
            </li>
          ))}
        </ul>
